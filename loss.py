...
def smooth_l1_loss(true, pred):
    ...
    lossx = face_label * tf.map_fn(smooth_l1, tf.reshape(gxs - logitx, (-1, 320)))
    lossy = face_label * tf.map_fn(smooth_l1, tf.reshape(gys - logity, (-1, 320)))
    lossw = face_label * tf.map_fn(smooth_l1, tf.reshape(gws - logitw, (-1, 320)))
    lossh = face_label * tf.map_fn(smooth_l1, tf.reshape(ghs - logith, (-1, 320)))
